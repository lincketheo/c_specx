...
TEST (ht_insert_idx_regression_trigger_swap)
{
  ...
  // the skipped value
  hash_table_idx ht;
  hentry_idx entries[TEST_TABLE_LEN];
  ht_init_idx (&ht, entries, TEST_TABLE_LEN);

  for (u32 i = 0; i < TEST_TABLE_LEN; ++i)
    {
      test_assert_int_equal (ht_insert_idx (&ht, (hdata_idx){ .key = i, .value = i }),
                             HTIR_SUCCESS);
    }
  ...
  test_assert_int_equal (
      ht_insert_idx (&ht, (hdata_idx){ .key = TEST_TABLE_LEN, .value = 0 }),
      HTIR_SUCCESS);
  test_assert_int_equal (
      ht_insert_idx (&ht, (hdata_idx){ .key = TEST_TABLE_LEN + 1, .value = 1 }),
      HTIR_SUCCESS);

  test_assert_int_equal (ht_insert_idx (&ht, (hdata_idx){ .key = 0, .value = 0 }),
                         HTIR_SUCCESS);
  ...
  if (has_bug)
    {
      ...
      test_assert_int_equal (ht.elems[3].data.key, TEST_TABLE_LEN + 1);
  ...
  else
    {
      ...
      test_assert_int_equal (ht.elems[2].data.key, TEST_TABLE_LEN + 1);
...
TEST (robin_hood_ht)
{
  ...
  test_assert_int_equal (ht_insert_idx (&ht, (hdata_idx){ .key = 10u, .value = 5 }),
                         HTIR_SUCCESS);

  TEST_CASE ("Duplicate insert")
  {
    test_assert_int_equal (
        ht_insert_idx (&ht, (hdata_idx){ .key = 10u, .value = 99 }), HTIR_EXISTS);
  }
  ...
  {
    test_assert_int_equal (
        ht_insert_idx (&ht, (hdata_idx){ .key = 10u, .value = 10 }), HTIR_SUCCESS);
    test_assert_int_equal (
        ht_insert_idx (&ht, (hdata_idx){ .key = 110u, .value = 11 }),
        HTIR_SUCCESS);
    test_assert_int_equal (
        ht_insert_idx (&ht, (hdata_idx){ .key = 210u, .value = 21 }),
        HTIR_SUCCESS);
  }
  ...
  TEST_CASE ("Full table")
  {
    ...
    for (u32 k = 0; k < 4; ++k)
      {
        test_assert_int_equal (
            ht_insert_idx (&tiny, (hdata_idx){ .key = k, .value = k }),
            HTIR_SUCCESS);
      }

    test_assert_int_equal (
        ht_insert_idx (&tiny, (hdata_idx){ .key = 99u, .value = 99 }), HTIR_FULL);
  }
}
#endif
